...
void getAngle()
{
  ...
  {
  theta = (360 + turnAngleDegrees);
  }
  if(turnAngleDegrees >= 0 && turnAngleDegrees <= 180)
  {
  theta = turnAngleDegrees;
  }
  lcd.clear();
  lcd.gotoXY(0,0);
  lcd.print("THETA:");
...
void getPosition()
{
  ...
  deltax = (cos(W)*(R*sin(globalTheta)))-(sin(W)*(-R*cos(globalTheta)))+R*sin(globalTheta);
  deltay = (sin(W)*(R*sin(globalTheta)))+(cos(W)*(-R*cos(globalTheta)))+R*cos(globalTheta);
...
void turnR(int desiredAngle)
{
  ...
  rotatCheck = false;
  if(theta + desiredAngle < -360 )
  {
    theta = (theta + desiredAngle - 360);
  }
  else
  {
    theta = (theta + desiredAngle);
  }
  getVelocity();
}
